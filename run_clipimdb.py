...
from processor.dataset import clipdataset, logger
...
from module.train import clipweightTrainer, clipweightClassifier
# from module.train_1 import clipweightTrainer, clipweightClassifier
...
def main():
    ...
    parser.add_argument('--model_name', default='dandelin/vilt-b32-mlm', type=str,
    ...
    parser.add_argument('--kg_dim', default=512, type=int)
    ...
    if args.save_path is not None:  # 创建保存路径目录
    ...
    if args.modelclass == 'clipweight':
        ...
        # model = GatedClassifier(args.vis_dim, args.text_dim, args.kg_dim, args.n_classes, args.hidden_size)
        # for name, param in model.named_parameters():
        #     print(name)
    # pdb.set_trace()
    trainer = Trainer(train_data=train_dataloader, dev_data=dev_dataloader, test_data=test_dataloader, model=model,
                      clipmodel=clipmodel, args=args, logger=logger, writer=writer,
